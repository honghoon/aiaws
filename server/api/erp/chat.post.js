...
export default defineEventHandler(async (event) => {
  ...
  if (result === 0) {
    return streamFallbackMessage(event, '죄송합니다. 해당 요청은 지원되지 않거나 인식할 수 없습니다.');
  }
  ...
  if (result === 1) {
    ...
    if (resultDataSet == null || resultDataSet.length == 0){
    ...
    await streamFallbackMessageJump(writer, '데이터 입니다. ')
    await streamFallbackMessageJump(writer, '--JSON--')
