<template>
        ...
        <div class="min-h-[calc(100vh-75px)] bg-white flex flex-col p-6">
            <div class="flex items-center justify-between">
                <div class="flex pb-3 justify-end gap-3 items-center ">
                    <n-select placeholder="처리자 검색 조건.." v-model:value="searchUser" :clearable="true" bordered="false" class="n-select-nowrap" style="width:300px" value-field="name" label-field="name" multiple :options="users" />
                ...
                <div class="flex pb-3 justify-end gap-3 items-center ">
                    ...
                    <n-button strong secondary round type="tertiary" @click="registerCard">
                    등록
                    </n-button>
                            ...
                            <div class="flex items-center justify-between pt-3 pb-3">
                                ...
                                <div class="flex gap-2 items-center">
                                    <div class="px-3 h-6 rounded-full text-xs font-semibold flex items-center"
                                        ...
                                        <n-icon class="ml-2 cursor-pointer" @click="()=>{openSelectItem(card)}"><OpenOutline/></n-icon>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <template #menu>
                ...
                <div
                    ...
                    <div class="relative w-250">
                        <div class="flex flex-col h-[400px] bg-slate-100/50 rounded-md text-sm text-slate-600 font-normal p-3">sdfsdf</div>
    ...
    <!-- 등록 / 수정 팝업 모달 -->
    <n-modal v-model:show="showModal" preset="card" :title="selectedItem?.type" class="w-[800px] max-w-[800px] rounded-md">
        <div  v-if="!updateMode" class="flex flex-col gap-2 p-3 rounded-md bg-slate-100">
            <div class="w-full bg-white p-3 rounded-md">{{ selectedItem?.title }}</div>
            <div class="w-full bg-white p-3 rounded-md h-100" v-html="selectedItem?.content"></div>
            <n-descriptions label-placement="left" class="w-full bg-white p-3 rounded-md">
                <n-descriptions-item>
                    <template #label>
                        생성일자
                    </template>
                    ...
                    <div v-if="selectedItem?.procName" class="text-green-400 flex-none rounded-full p-1']">
        ...
        <n-form label-placement="left" :label-width="50" size="small" ref="formRef" v-if="updateMode" :model="selectedItem" class="rounded-md p-3">
            <n-form-item :span="2"  label="처리자" path="procName">
                <n-select v-model:value="selectedItem.procName" :options="users" value-field="name" label-field="name"
                    placeholder="처리자를 선택하세요" clearable />
            </n-form-item>
            <n-form-item  :span="2"  label="상태" path="status">
                <n-select v-model:value="selectedItem.status" :options="status" placeholder="상태를 선택하세요" value-field="name" label-field="name"
                    clearable />
            </n-form-item>
            <n-form-item label="유형" path="type">
                <n-select v-model:value="selectedItem.type" :options="Object.keys(mappings).map(type => ({ label: type, value: type }))"
                    placeholder="유형을 선택하세요" clearable />
            </n-form-item>
            <n-form-item label="제목" path="title">
                <n-input v-model:value="selectedItem.title" placeholder="제목을 입력하세요" clearable />
            </n-form-item>
            <n-form-item label="내용" path="content">
                ...
                <div class="sticky top-0 z-10 flex gap-2 bg-white py-2 px-4 border-b border-gray-200">
                    ...
                    <!-- 밑줄: removeOutline(대체) -->
                    <n-button quaternary size="small" @click="editor.chain().focus().toggleUnderline().run()">
        ...
        <template #action>
            <div class="flex p-2 gap-3 items-center justify-end">
                <n-button strong secondary v-if="!updateMode" @click="updateMode = true">
                    편집
                </n-button>
                <n-button strong secondary type="warning" v-if="updateMode" @click="updateMode = false">
                    취소
                </n-button>
                <n-button strong  type="primary" secondary v-if="updateMode" @click="()=>{showModal = false}" >
                    저장
                </n-button>
...
const cards = ref([
  {
    id: 1,
    type: "개발",
    title: "분석 설계 안녕하세요. 안녕하세요. 제목입니다. .. 으흐흐흐 하나 둘 셋 넷 다섯.. 여섯 . 으흐흐흐 하나 둘 셋 넷 다섯.. 여섯 . 으흐흐흐 하나 둘 셋 넷 다섯.. 여섯 ",
    status: "대기 업무",
    date: "2025-07-01",
    content: `<ul>
