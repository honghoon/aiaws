<template>
    <div class="relative">
        ...
        <n-float-button position="fixed" bottom="50px" right="24px" menu-trigger="click" v-model:show-menu="showMenu"
            ...
            <n-icon>
                <!-- AI 기능에 적합한 간단한 'AI' 텍스트 아이콘 -->
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    aria-label="AI 버튼">
                    <text x="12" y="16" text-anchor="middle" font-size="12" fill="currentColor">AI</text>
                </svg>
            </n-icon>

            <template #menu>
                <!-- 하단 중앙 고정 입력창 (아이콘 버튼 포함) -->
                <div class="fixed inset-x-0 bottom-0 bg-white p-4 shadow-xl flex justify-center">
                    <div class="relative w-250">
                        <textarea v-model="aiText" rows="2"
                            class="w-full resize-none rounded border px-3 py-2 pr-10 text-sm focus:outline-none focus:ring"
                            @input="autoResize" placeholder="AI에게 물어보세요..."
                            :style="{ 'max-height': textareaMaxHeight + 'px' }" />

                        <!-- 보내기 버튼 (아이콘) -->
                        <button @click="submitAI"
                            class="absolute bottom-2 right-2 flex items-center justify-center w-6 h-6 bg-blue-600 text-white rounded-full hover:bg-blue-700">
...
// 카드 데이터
const cards = ref([
    { id: 1, type: "개발", title: '분석 설계 안녕하세요. 안녕하세요. 제목입니다. .. 으흐흐흐 하나 둘 셋 넷 다섯.. 여섯 . 으흐흐흐 하나 둘 셋 넷 다섯.. 여섯 . 으흐흐흐 하나 둘 셋 넷 다섯.. 여섯 ', status: '대기 업무', date: "2025-07-01", procName: "홍훈" },
    { id: 2, type: "개발", title: '프론트 개발', status: '대기 업무', date: "2025-07-01", procName: "홍훈" },
    { id: 3, type: "업무협의", title: '테스트 작성', status: '대기 업무', date: "2025-07-01", procName: "홍훈" }
])
